...
interface counter_if(input clk);
	...
	// 1. property:to be counter value 0 occurupted after reset 0 when posedge clk
	// |=> :
...
class counter_sequence extends uvm_sequence #(counter_seq_item);
	...
	virtual task body();
		...
		//reset test: rst_n = 0, enable = 1'b0
		repeat(3)begin
		...
		repeat(100) begin
			// new counter seq item
			counter_seq_item c_item;
			c_item = counter_seq_item::type_id::create("c_item");
			// randomized()
			start_item(c_item);
			if(!c_item.randomize()) begin
				`uvm_fatal(get_type_name(), "c_item_randomized fail" )
			end
			c_item.rst_n = 1;
			finish_item(c_item);
		end
...
class counter_scoreboard extends uvm_scoreboard;
	...
	virtual function void write(counter_seq_item c_item);
		...
		if(!c_item.rst_n) begin
			`uvm_info(get_type_name(), $sformatf("To skip !! expect:%d, counter:%d", expected_count, c_item.counter), UVM_HIGH)
			skip_cnt++;
			expected_count = 0;
			expected_o_tick = 0;
		end else begin
			//pass / fail
			if(expected_count == c_item.counter && expected_o_tick == c_item.o_tick)begin
				`uvm_info(get_type_name(), $sformatf("Pass !! expect:%d, counter:%d", expected_count, c_item.counter), UVM_HIGH)
				pass_cnt++;
			end
			else begin
				`uvm_info(get_type_name(), $sformatf("Fail !! expect:%d, counter:%d", expected_count, c_item.counter), UVM_HIGH)
				fail_cnt++;
			end
			expected_o_tick = 0;
			if(c_item.enable)begin
				expected_count++;
				if(c_item.counter == 7)begin
					expected_o_tick = 1;
				end
			end
		end
